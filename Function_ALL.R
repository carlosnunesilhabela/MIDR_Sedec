...
std_histdesp <- function(dsname) {
  ...
  # despesas_acum <- read_xlsx(dsname) # opcional para alterar direto no arquivo xlsx
  despesas_f    <- dsname
  ...
  despesas_f$historico_std <- tolower(despesas_f$historico_despesa)    # coloca tudo em lowercase - para Uppercase seria toupper(str_origem) 
  ...
  write.xlsx(despesas_f, "Despesas_municipios2.xlsx") #grava data frame em formato *.xlsx
